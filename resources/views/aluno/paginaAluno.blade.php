...
<body>
    ...
    <header class="cabecalho-topo">
        <div class="container">
            ...
            <nav class="botoes-acao">
                @if(auth()->user()->tipo == 1)
                <a href="{{ route('evento.index') }}" class="link-verificar">Voltar ao Painel</a>
                @endif
                ...
                <form action="{{ route('logout') }}" method="POST" style="display:inline;">
                    @csrf
                    <button type="submit" class="botao-admin" style="background-color: var(--vermelho-erro); border-color: var(--vermelho-erro); cursor: pointer;">
                        Sair
                    </button>
                </form>
                @endif
    ...
    <main class="container container-eventos">
        ...
        <div class="dashboard-header" style="margin-bottom: 30px;">
            <h1 style="font-family: 'SN Pro', sans-serif; color: var(--azul-mais-escuro); font-size: 26px; font-weight: 800; margin-bottom: 6px;">
                Olá, {{ auth()->user()->nome }}!
            </h1>
        ...
        @if(session('success'))
        <div class="container-sem-eventos" style="border-left: 4px solid var(--verde-principal); margin-bottom: 25px; padding: 15px; text-align: left;">
        ...
        <div class="cartao-evento">
            ...
            @if($certificado)
            <div class="info-evento" style="border-left-color: var(--verde-principal); display: flex; justify-content: space-between; align-items: center; background-color: rgba(29, 215, 60, 0.05); margin-bottom: 20px;">
                <span style="color: #117a22; font-weight: 700; font-size: 14px;">✓ Certificado disponível para download</span>
            ...
            <div class="info-evento">
                ...
                <p>
                    ...
                    {{ date('d/m/Y', strtotime($evento->data_inicio)) }} até {{ date('d/m/Y', strtotime($evento->data_fim)) }}
                </p>
            </div>
                ...
                <table class="tabela-estilizada">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><span class="item-badge" style="background-color: var(--fundo-claro); color: var(--azul-mais-escuro);">{{ $atividade->tipo }}</span></td>

                            @if(auth()->user()->tipo == 2)
                            <td class="coluna-acao">
                                @if($atividadeEncerrada)
                                <span style="color: var(--texto-secundario); font-size: 13px; font-weight: 500;">Participação encerrada</span>
                                @else
                                <form action="{{ route('inscricao.destroy', $inscricao->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="botao-inscrever" style="background-color: var(--vermelho-erro); cursor: pointer; border: none;">
                                        Desinscrever-se
                                    </button>
                                </form>
                                @endif
                        ...
                        <tr>
                            ...
                            <td class="coluna-acao">
                                ...
                                <form action="{{ route('inscricao.store') }}" method="POST">
                                    ...
                                    <button type="submit" class="botao-inscrever" style="cursor: pointer; border: none;">
                                        Inscrever-se
                                    </button>
                                </form>
                                @else
